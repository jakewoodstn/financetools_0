...
function changeSplit() {
    ...
    if (parseFloat(tot).toFixed(2) == parseFloat($('#splitTotal').text()).toFixed(2)) {
        $('#splitAssigned').removeClass('unbalanced');
        $('#splitRemaining').addClass('balancedHidden');
        $('#splitRemaining').removeClass('unbalanced');
    } else {
        $('#splitRemaining').find('span').text(parseFloat(parseFloat($('#splitTotal').text()) - parseFloat(tot)).toFixed(2));
