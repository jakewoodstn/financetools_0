    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                if (p.dataType == 'json') {
                    data = $.extend({ rows: data, page: 1, total: (data==null)?0:data.length }, data);
                }
                if (p.preProcess) {
                        ...
                        $('thead tr:first th', g.hDiv).each( //add cell
							function () {
							    ...
							    if (typeof row.cell == 'undefined') {
							        if (typeof row[p.colModel[idx].name] == 'undefined') { td.innerHTML = (row[idx] != null) ? row[idx] : ''; }
							        else { td.innerHTML = row[p.colModel[idx].name]; }
							    } else {
